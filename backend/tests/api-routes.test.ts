...
describe('Credit Pool Routes', () => {
  it('GET /credit-pool returns pool status', async () => {
    ...
    expect(res.statusCode).toBe(200);
    const body = res.json();
    expect(body.totalBalanceUsd).toBe(1000);
    expect(body.totalAllocatedUsd).toBe(250);
    expect(body.availableUsd).toBe(750);
    expect(body.reservedUsd).toBe(100);
    expect(body.reservePct).toBe(10);
    expect(body.lastUpdated).toBe('2025-01-01T00:00:00Z');
    expect(deps.creditPoolService.getPoolState).toHaveBeenCalledOnce();
    await app.close();
  });
});
