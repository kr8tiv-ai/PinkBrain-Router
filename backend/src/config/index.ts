...
const configSchema = z.object({
  ...
  maxKeyLimitUsd: z.coerce.number().min(1).max(10000).default(100),
  keyRotationDays: z.coerce.number().min(1).max(365).default(90),
  usagePollIntervalMin: z.coerce.number().min(1).max(1440).default(15),
...
export function loadConfig(): Config {
  ...
  const rawConfig = {
    ...
    maxKeyLimitUsd: process.env.MAX_KEY_LIMIT_USD,
    keyRotationDays: process.env.KEY_ROTATION_DAYS,
    usagePollIntervalMin: process.env.USAGE_POLL_INTERVAL_MIN,
